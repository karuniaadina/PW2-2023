...
@section('content')
    ...
    <div class="content-wrapper">
        ...
        <div class="card">
            <div class="card-body">
                <h3>Edit reviews</h3><br />
                <form class="forms-sample" action="{{ url('/admin/reviews', $reviews->id) }}" method="POST">
                    @csrf
                    @method('put')
                    <div class="form-group row">
                        ...
                        <div class="col-sm-8">
                            <input type="date" class="form-control" id="date" name="date"
                                placeholder="Date reviews" value="{{ $reviews->date }}">
                            @if (count($errors) > 0)
                            ...
                            <select class="form-control bg-white text-dark" id="users_id" name="users_id">
                                ...
                                @foreach ($users as $item)
                                    <option value="{{ $item->id }}"
                                        {{ $reviews->users_id == $item->id ? 'selected' : '' }}>{{ $item->name }}
                                    </option>
                                @endforeach
                            </select>
                            @if (count($errors) > 0)
                        ...
                        <div class="col-sm-8">
                            <select class="form-control bg-white text-dark" id="movies_id" name="movies_id">
                                ...
                                @foreach ($anime as $item)
                                    <option value="{{ $item->id }}"
                                        {{ $reviews->movies_id == $item->id ? 'selected' : '' }}>{{ $item->title }}
                                    </option>
                                @endforeach
                            </select>
                            @if (count($errors) > 0)
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-sm-8">
                            <input type="number" min="1" max="5" class="form-control" id="rating"
                                name="rating" placeholder="Rating reviews" value="{{ $reviews->rating }}">
                            @if (count($errors) > 0)
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-sm-8">
                            <textarea class="form-control" id="description" name="description" placeholder="Description reviews"
                                value="{{ $reviews->description }}"> {{ $reviews->description }}</textarea>
                            @if (count($errors) > 0)
